...
public class ImagePHash {
	...
	public static String getHash(BufferedImage read) {
		...
		StringBuilder tempBinaryHash = new StringBuilder();
		int i = 0;
		for (int x = 0; x < SMALLER_SIZE; x++) {
			for (int y = 0; y < SMALLER_SIZE; y++) {
				if (x != 0 && y != 0) {
					tempBinaryHash.append(dctValues[x][y] > avg ? "1" : "0");
					i++;
					if (i % 8 == 0) {
						String hex = Integer.toHexString(Integer.parseInt(tempBinaryHash.toString(), 2));
						hash.append(hex);
						tempBinaryHash = new StringBuilder();
					}
				}
			}
		}
